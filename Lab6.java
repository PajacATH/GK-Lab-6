...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {
		...
		gl2.glRotatef(45, 0, 1, 0); //Rotacja o 45 stopni by podstawa pasowa³a do piramidy 
		glut.glutSolidCube(17); //Minimalne zmniejszy³em wielkoœæ podstawy by piramida nie by³a tak du¿a
		gl2.glPopMatrix();
		...
		float x,z,x1,z1;
		int i; //Do pêtli rysuj¹cej boki
		int n = 4; //Iloœæ boków 
		int color=1; //Pozwala zmieniæ kolor
		int size =10;
		float y = -1; //Wysokoœæ na jakiej startuje rysowanie
		x1 = (float) Math.sin(0)*size;
		z1= (float) Math.cos(0)*size;
		
		//Dodawanie materia³ów i ich wartoœci
		
		gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT, materials[color], 7); // 0 
		gl2.glMaterialfv( GL2.GL_FRONT_AND_BACK, GL2.GL_DIFFUSE, materials[color], 7 ); // 7
		gl2.glMaterialfv( GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR, materials[color], 10 ); // 10 i 12
		gl2.glMaterialf( GL2.GL_FRONT_AND_BACK, GL2.GL_SHININESS, materials[color][12] ); 
		
		for (i = 1; i<= n; i += 1) { //Pêtla do rysowania boków
			gl2.glVertex3f(x1, y, z1);
			x = (float) Math.sin(i * 2 * Math.PI / n)*size;
			z = (float) Math.cos(i * 2 * Math.PI / n)*size;
			x1=x;
			z1=z;
			gl2.glVertex3f(x, y, z);
			gl2.glVertex3f(0, 8, 0); //Wartoœc y odpowiedzialna za wysokoœæ
			}
		gl2.glEnd();
		gl2.glFlush();
		//Koniec rysowania piramidy
		
	//Tworzenie zród³a œwiat³a
		
    	//Dodanie pseudo s³oñca
	
		  float sun[] = { 1F, 1, 0, 1 };
		  gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR, gray, 0); 
          gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_EMISSION, sun, 0);  //Sprawiaj¹ ¿e kulka jest ¿ó³ta
          gl2.glTranslated(8, 6, 0); //Ustawia s³oñce w przestrzeni
          gl2.glEnable(GL2.GL_LIGHT1); //Zaczyna emitowaæ ¿ó³te œwiat³o

          glut.glutSolidSphere(3, 16, 8); //Wielkoœæ kulki
          gl2.glPopMatrix();
          
          //Dodanie ciemno ¿ó³tej kuli
          
          float darkYellow[] = {0.5f, 0.5f, 0.0f, 1.0f};
          gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_EMISSION, darkYellow, 0); 
          gl2.glEnable(GL2.GL_LIGHT2);
          gl2.glPushMatrix();
          gl2.glTranslated(-6, 2, 7);
          gl2.glLightfv(GL2.GL_LIGHT2, GL2.GL_POSITION, zero, 0);
          glut.glutSolidSphere(2, 16, 8);
